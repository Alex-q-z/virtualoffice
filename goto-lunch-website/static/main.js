...
function updateActiveUsers() {
  ...
  for (let sid in active_users) {
    ...
    if (active_users[sid]["user_id"] == global_username) {
...
function serverConnect() {
  ...
  socket.connect();
  socket.emit("new_user_connect_to_server", global_user_info);
...
function updateDoNotDisturb() {
  socket.emit("update_do_not_disturb", global_user_info["do_not_disturb"]);
}

function noDisturbCheckboxChange() {
  global_user_info["do_not_disturb"] = noDisturbCheckbox.checked;
  if (user_online_flag) {
    updateDoNotDisturb();
  }
}
