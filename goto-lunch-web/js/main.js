...
function start() {
  console.log('in start(), but we do not do anything');
...
function call() {
  // we connect to socket first
  console.log('in call(): before socket.connect()');
  socket.connect();
...
function upgrade() {
  ...
  navigator.mediaDevices
      ...
      .then(stream => {
        localStream = stream; // QZ added
        localVideo.srcObject = stream; // QZ added
        ...
        // QZ: addTrack so remote client can see local streams
        // const videoTracks = stream.getVideoTracks();
